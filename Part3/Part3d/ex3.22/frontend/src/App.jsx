    ...
    //Else if the person exists BUT number is different, allow user to change the number
      ...
      if (window.confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)) {
        ...
        personServices
          ...
          .then(newData => {
            console.log('Response from update:', newData)  // Log the response data
            console.log('Updating persons state with new data')
            setPersons(persons.map(person => person.id === existingPerson.id ? newData : person))
            setNewName('')
            setNewNumber('')
          })
          .catch(error => {
            console.log('Update error:', error.response.data || error)  // Log more error details
