...
describe('Part 4, Blog Table Unit Tests', () => {
  ...
  describe('Ex4.19: Posting Blogs with Token Authentication', () => {
    ...
    test('Ex4.19: Testing failure if token is invalid', async () => {
      const newBlog = {
      ...
      const response = await api.post('/api/blogs')
        ...
        .expect(401)
        .expect('Content-Type', /application\/json/)
      assert.match(response.body.error, /Invalid token/)
  ...
  describe('Ex4.21: Deleting Blogs', () => {
    test.only('Ex4.21: Deleting an existing blog with creater\'s credential works', async () => {
      const blogsAtStart = await helper.blogsInDb()
      const blogToDelete = blogsAtStart[0]
      await api.delete(`/api/blogs/${blogToDelete.id}`)
        .set('Authorization', `Bearer ${token}`)
    ...
    test.only('Ex4.21: Deleting a blog that does not belong to the user should throw 401 Unauthorized error', async () => {
